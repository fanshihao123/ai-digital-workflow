...
for c in start hotfix review test status resume pause continue init answer fix; do
...
echo ""
echo "  下一步："
echo "    1. cd $PROJECT_ROOT && claude /init"
echo "       ↑ AI 自动分析项目，生成 CLAUDE.md + rules/（推荐）"
echo "       或手动编辑 .claude/*.md 填入项目信息"
echo "    2. vim .env.ai-digital-workflow  # 直接编辑，无需复制"
echo "    3. 测试: opencli claude --permission-mode bypassPermissions -p '/start 添加用户登录'"
echo "=================================================="
