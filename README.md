...
| 命令 | 动作 |
|------|------|
| `/start <描述>` | 启动完整流水线（别名: `/start-workflow`, `/workflow`） |
| `/hotfix <描述>` | 跳过设计，快速修复（跳过澄清和审查阻断） |
| `/pause [feature]` | 主动暂停工作流，保存断点 |
| `/continue [feature] [变更描述]` | 从 `/pause` 继续，自动 diff 需求并最小粒度更新（别名: `/restart`） |
| `/resume [feature]` | 从崩溃/意外中断恢复，读日志找断点继续 |
| `/answer <需求名> <答复>` | 答复 Step 1 开放问题，恢复暂停的工作流 |
| `/fix <需求名> <修改指导>` | 答复 Spec 审查严重问题，Claude 自动修改并重新审查（别名: `/fix-spec`） |
| `/review [feature]` | 单独触发代码审查 |
| `/test [feature]` | 单独触发测试 |
| `/status` | 查看流水线进度、Git log、扩展状态 |
| `/deploy [feature]` | 手动触发部署 |
| `/rollback` | 回滚部署 |
| `/init` | AI 分析项目，智能生成 CLAUDE.md + rules/（别名: `/init-knowledge`） |
...
回复: /fix user-auth 密码必须用 bcrypt 哈希存储
```
...
- **命令简化**: `/start-workflow` → `/start`、`/restart` → `/continue`、`/fix-spec` → `/fix`、`/init-knowledge` → `/init`（旧名保留为兼容别名）
- **补齐缺失命令定义**: 新增 `commands/fix.md`、`commands/deploy.md`、`commands/rollback.md`（之前只有 `.sh` 无 `.md`，Claude Code 无法识别为斜杠命令）
