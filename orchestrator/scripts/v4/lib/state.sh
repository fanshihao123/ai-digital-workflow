...
# 供 v3→v4 过渡期使用
# ============================================================
state_migrate_from_v3() {
  local feature="$1"
  local spec_dir="$PROJECT_ROOT/specs/$feature"
  local file
  file=$(_state_file "$feature")
  ...
  if [ -f "$spec_dir/awaiting-spec-review.json" ] && \
     jq -e '.status == "awaiting"' "$spec_dir/awaiting-spec-review.json" >/dev/null 2>&1; then
    status="awaiting-fix"
    step=1
    resume_cmd="/fix-spec"
    local cc; cc=$(jq -r '.critical_count // 0' "$spec_dir/awaiting-spec-review.json" 2>/dev/null)
    context=$(jq -n --argjson cc "$cc" '{critical_count:$cc}')

  elif [ -f "$spec_dir/awaiting-clarification.json" ] && \
       jq -e '.status == "awaiting"' "$spec_dir/awaiting-clarification.json" >/dev/null 2>&1; then
    status="awaiting-answer"
    step=1
    resume_cmd="/answer"
    local qs; qs=$(jq -r '.questions // ""' "$spec_dir/awaiting-clarification.json" 2>/dev/null)
    context=$(jq -n --arg qs "$qs" '{questions:$qs}')

  elif [ -f "$spec_dir/paused.json" ] && \
       jq -e '.status == "paused"' "$spec_dir/paused.json" >/dev/null 2>&1; then
    status="paused"
    step=$(jq -r '.paused_step // 1' "$spec_dir/paused.json" 2>/dev/null)
    resume_cmd="/restart"
    context=$(jq -n --argjson s "$step" '{paused_step:$s}')
  fi
  ...
  if [ "$status" != "idle" ]; then
    jq -n \
      ...
      '{feature:$f, status:$s, step:$st, substep:"migrated-from-v3", updated_at:$t, resume_command:$rc, context:$ctx}' \
      > "$file"
    echo "  [state] 已从 v3 状态文件迁移: $feature → $status (step $step)" >&2
  fi
}
